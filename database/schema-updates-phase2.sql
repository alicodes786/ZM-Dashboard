-- Phase 2 Schema Updates: Adding Margin Calculation Support
-- Run this after the main schema.sql

-- Add margin-related fields to daily_work_entries
ALTER TABLE daily_work_entries 
ADD COLUMN IF NOT EXISTS client_rate DECIMAL(10,2), -- What we charge the client per hour
...
-- Add computed client_cost field (for clarity)
ALTER TABLE daily_work_entries 
ADD COLUMN IF NOT EXISTS client_cost DECIMAL(10,2); -- Total amount charged to client

-- Function to calculate margin and client cost
...
BEGIN
    ...
    IF NEW.override_cost IS NOT NULL THEN
        -- Admin set a specific price
        v_client_cost := NEW.override_cost;
    ELSIF NEW.client_rate IS NOT NULL THEN
        -- Calculate based on client hourly rate
        v_client_cost := NEW.hours_worked * NEW.client_rate;
    ELSE
        -- Fallback: use labor cost (no margin)
...
-- Function to get daily margin summary
